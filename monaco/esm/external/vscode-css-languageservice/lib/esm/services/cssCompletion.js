...
import { isColorString, colors, colorKeywords, colorFunctions, isColorConstructor } from '../languageFacts/colors.js';
import { cssWideKeywords, cssWideFunctions, units, positionKeywords, repeatStyleKeywords, lineStyleKeywords, lineWidthKeywords, geometryBoxKeywords, boxKeywords, imageFunctions, transitionTimingFunctions, basicShapeFunctions, html5Tags, svgElements } from '../languageFacts/builtinData.js';
import { startsWith, endsWith, getLimitedString } from '../utils/strings.js';
...
import { InsertTextFormat, Range, Position, CompletionItemKind, TextEdit, CompletionItemTag, MarkupKind } from '../../../../vscode-languageserver-types/lib/esm/main.js';
...
class CSSCompletion {
    ...
    getUnitProposals(entry, existingNode, result) {
        ...
        if (entry.restrictions) {
            for (const restriction of entry.restrictions) {
                const units$1 = units[restriction];
                if (units$1) {
                    for (const unit of units$1) {
